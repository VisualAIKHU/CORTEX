...
class CrossTransformer(nn.Module):
    ...
    def forward(self, input1, input2, dec_mask, diagonal_mask=True):
        ...
        mask = (torch.triu(torch.ones(tgt_length, tgt_length, device=device)) == 1).transpose(0, 1)
...
class TransformerDecoder(nn.Module):
    ...
    def sample(
        ...
        for dec_idx in range(max_cap_len):
            ...
            decoder_outputs, attention_weight, sim_loss = self.core(text_input_ids, text_masks, encoder_output, diagonal_mask=True)
