...
parser.add_argument('--exp_dir', type=str, default='./experiments3')
...
# snapshot_full_path = args.snapshot
...
with torch.no_grad():
    ...
    for i, batch in tqdm(enumerate(test_loader)):
        ...
        encoder_output_pos, cortex_loss_pos, img_to_txt_loss_pos = change_detector(d_feats, sc_feats, c_d_feats, c_sc_feats)
        encoder_output_neg, cortex_loss_neg, img_to_txt_loss_neg = change_detector(d_feats, nsc_feats, c_d_feats, c_n_feats)

        speaker_output_pos, pos_att = speaker.sample(encoder_output_pos, sample_max=1)
        speaker_output_neg, neg_att = speaker.sample(encoder_output_neg, sample_max=1)
