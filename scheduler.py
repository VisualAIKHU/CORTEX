...
class GradualWarmupScheduler(optim.lr_scheduler._LRScheduler):
    ...
    def __init__(self, optimizer, total_epoch:int,multiplier=1, after_scheduler_cfg=None):
...
def build_scheduler(scheduler_cfg:Union[EasyDict, None], optimizer)->optim.lr_scheduler._LRScheduler:
    ...
    if scheduler_cfg.type_name.lower() == 'StepLR'.lower():
        return optim.lr_scheduler.StepLR(optimizer, **scheduler_cfg.keywords) #step_size
    if scheduler_cfg.type_name.lower() == 'MultiStepLR'.lower():
        return optim.lr_scheduler.MultiStepLR(optimizer, **scheduler_cfg.keywords) #milestones
    if scheduler_cfg.type_name.lower() == 'ExponentialLR'.lower():
        return optim.lr_scheduler.ExponentialLR(optimizer, **scheduler_cfg.keywords) #gamma
    if scheduler_cfg.type_name.lower() == 'CosineAnnealingLR'.lower():
        return optim.lr_scheduler.CosineAnnealingLR(optimizer, **scheduler_cfg.keywords) #T_max
