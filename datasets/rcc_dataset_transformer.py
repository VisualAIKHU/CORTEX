...
class RCCDataset(Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        # 🔹 HDF5 파일 열기 (BefCap, AftCap)
        self.bef_cap_h5 = h5py.File(cfg.data.bef_cap_h5, 'r')
        self.aft_cap_h5 = h5py.File(cfg.data.aft_cap_h5, 'r')

        # 🔹 HDF5 데이터 구조 확인
        self.bef_embeddings = self.bef_cap_h5['embeddings']
        self.aft_embeddings = self.aft_cap_h5['embeddings']
        self.bef_start_end_idx = self.bef_cap_h5['start_end_idx'][:]
        self.aft_start_end_idx = self.aft_cap_h5['start_end_idx'][:]
    ...
    def __getitem__(self, index):
        ...
        # 🔹 Caption 임베딩 가져오기 (HDF5에서 start, end 인덱스 사용)
        bef_start, bef_end = self.bef_start_end_idx[img_idx]
        aft_start, aft_end = self.aft_start_end_idx[img_idx]

        bef_caps = torch.FloatTensor(self.bef_embeddings[bef_start:bef_end])  # shape: (n_bef_caps, 768)
        aft_caps = torch.FloatTensor(self.aft_embeddings[aft_start:aft_end])  # shape: (n_aft_caps, 768)
        nsc_caps = torch.FloatTensor(self.bef_embeddings[bef_start:bef_end])  # shape: (n_aft_caps, 768)
        ...
        return (d_feature, n_feature, q_feature, bef_caps, aft_caps, nsc_caps,
    ...
    def close(self):
        """HDF5 파일 닫기."""
        self.bef_cap_h5.close()
        self.aft_cap_h5.close()
...
def rcc_collate(batch):
    ...
    # BefCap 및 AftCap의 가변 길이 텐서 처리
    bef_caps_batch = transposed[3]
    aft_caps_batch = transposed[4]
    nsc_caps_batch = transposed[5]

    seq_batch = default_collate(transposed[6])
    label_with_ignore_batch = default_collate(transposed[7])
    neg_seq_batch = default_collate(transposed[8])
    neg_label_with_ignore_batch = default_collate(transposed[9])
    mask_batch = default_collate(transposed[10])
    neg_mask_batch = default_collate(transposed[11])
    aux_label_pos_batch = default_collate(transposed[12])
    aux_label_neg_batch = default_collate(transposed[13])
    ...
    d_img_batch = transposed[14]
    n_img_batch = transposed[15]
    q_img_batch = transposed[16]
    
    return (d_feat_batch, n_feat_batch, q_feat_batch,
            bef_caps_batch, aft_caps_batch, nsc_caps_batch,
